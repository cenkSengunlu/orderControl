            ...
            <div className='flex justify-between'>
              <div className='flex flex-col'>
                <div className=' w-32 h-18'>
                  {!sizeChecked &&
                    <ContentHeightWidth index={index + 1} />
                  }

                  {sizeChecked &&
                    <div className='h-16'>
                      <select className='w-full px-3 py-3 mt-2' {...register(`${index + 1}-readySize`, {required: 'Ebad Seçiniz!'})}>
                      ...
                      <p className='text-red-500 font-semibold text-xs px-1 mt-1'>{errors[`${index + 1}-readySize`]?.message}</p>
                    </div>
                  }
                  ...
                  <div className='flex mt-1 justify-between'>
                    ...
                    <div className="checkBox flex items-center">
                      <label className="switch">
                        <input type="checkbox" {...register (`${index + 1}-readySizeToggle`)} onChange={() => handleChangeSize()}/>
                ...
                <div>
                  <select className='w-full px-2 py-3 mt-2 h-18' {...register(`${index + 1}-sticth`, {required: 'Dikiş Türü Seçiniz!'})}>
                  ...
                  <p className='text-red-500 font-semibold text-xs px-1 mt-1'>{errors[`${index + 1}-sticth`]?.message}</p>
                </div>
              ...
              <div className='flex flex-col w-24 mt-1'>
                <div className='flex flex-col'>
                  ...
                  <input {...register (`${index + 1}-piece`, {required: 'Ürün Adeti Giriniz!'})} className='rounded text-black py-0.5 px-2 w-full' type="text"/>
                  <p className='text-red-500 font-semibold text-xs px-1 mt-1'>{errors[`${index + 1}-piece`]?.message}</p>
                </div>
                ...
                <div className='flex mt-3 justify-between'>
                  ...
                  <div className="checkBox flex items-center">
                    <label className="switch">
                      <input type="checkbox" {...register (`${index + 1}-weightToggle`)} onChange={() => handleChangeWeight()}/>
                      <span className="slider round"></span>
                    </label>
                  </div>
                </div>
                <div>
                  <select className='w-full pl-2 py-3 mt-2 h-18' {...register(`${index + 1}-weight`, {
                  ...
                  <p className='text-red-500 font-semibold text-xs px-1 mt-1'>{weightChecked? errors[`${index + 1}-weight`]?.message : ''}</p>
                </div>
              </div>
